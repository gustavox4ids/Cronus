...
#ifdef NOISY
	ShowInfo("Pedido de save de cl„ ("CL_BOLD"%d"CL_RESET" - flag 0x%x).",g->guild_id, flag);
#endif
...
struct guild * inter_guild_fromsql(int guild_id)
{
...
#ifdef NOISY
	ShowInfo("Pedido de carregamento do cl„ %d\n", guild_id);
#endif
	...
	{	// Fix reduction of MAX_GUILD [PoW]
		ShowWarning("Cl„ %d:%s especificou mais usu·rios (%d) que MAX_GUILD (%d)\n", guild_id, g->name, g->max_member, MAX_GUILD);
...
int inter_guildcastle_tosql(struct guild_castle *gc)
{
	...
	if (SQL_ERROR == Sql_Query(sql_handle, StringBuf_Value(&buf)))
		...
		ShowInfo("Castelo do cl„ %d salvo\n", gc->castle_id);
...
static struct guild_castle* inter_guildcastle_fromsql(int castle_id)
{
	...
	if (save_log)
		ShowInfo("Castelo %d do cl„ %d carregado\n", castle_id, gc->guild_id);
...
int inter_guild_CharOnline(int char_id, int guild_id)
{
   ...
   int i;
	...
	if(!g) {
		ShowError("Cl„ %d do char %d n„o foi encontrado!\n", guild_id, char_id);
		return 0;
	}
...
int guild_calcinfo(struct guild *g)
{
	...
	if(g->max_member > MAX_GUILD)
	{
		ShowError("Cl„ %d:%s tem capacidade de muitos usu·rios (%d). O m·ximo suportado È %d\n", g->guild_id, g->name, g->max_member, MAX_GUILD);
		g->max_member = MAX_GUILD;
	}
...
int mapif_guild_created(int fd,int account_id,struct guild *g)
{
	...
	if(g != NULL)
	{
		...
		ShowInfo("int_guild: Cl„ criado (%d - %s)\n",g->guild_id,g->name);
	} else
		WFIFOL(fd,6)=0;
...
int mapif_guild_noinfo(int fd,int guild_id)
{
	...
	ShowWarning("int_guild: Sem informaÁıes do cl„ %d\n",guild_id);
	if(fd<0)
		mapif_sendall(buf,8);
	else
		mapif_send(fd,buf,8);
...
int mapif_guild_withdraw(int guild_id,int account_id,int char_id,int flag, const char *name, const char *mes)
{
	...
	ShowInfo("int_guild: Saque do cl„ (%d - %d: %s - %s)\n",guild_id,account_id,name,mes);
	return 0;
}
...
int mapif_guild_broken(int guild_id,int flag)
{
	...
	WBUFB(buf,6)=flag;
	mapif_sendall(buf,7);
	ShowInfo("int_guild: Cl„ quebrou (%d)\n",guild_id);
	return 0;
}
...
int mapif_parse_CreateGuild(int fd,int account_id,char *name,struct guild_member *master)
{
	...
	int i=0;
#ifdef NOISY
	ShowInfo("Criando cl„ (%s)\n", name);
#endif
	if(search_guildname(name) != 0){
		ShowInfo("int_guild: Cl„ com mesmo nome j· existe (%s)\n",name);
	...
	strcpy(g->position[0].name,"Mestre do Cl„");
	...
	for(i=1;i<MAX_GUILDPOSITION-1;i++) {
		sprintf(g->position[i].name,"PosiÁ„o %d",i+1);
	...
	if (!inter_guild_tosql(g,GS_BASIC|GS_POSITION|GS_SKILL)) {
		...
		ShowError("CriaÁ„o do cl„ %s falhou (Mestre do cl„: %s)\n", g->name, g->master);
		mapif_guild_created(fd,account_id,NULL);
		aFree(g);
		return 0;
	}
	ShowInfo("Cl„ %d - %s criada (Mestre do cl„: %s)\n", g->guild_id, g->name, g->master);
	...
	if(log_inter)
		inter_log("cl„ %s (id=%d) criado pelo mestre %s (id=%d)\n",
...
int mapif_parse_GuildMemberInfoChange(int fd,int guild_id,int account_id,int char_id,int type,const char *data,int len)
{
	...
	// Not Found
	if(i==g->max_member){
		ShowWarning("int_guild: GuildMemberChange: N„o foi encontrado %d:%d no cl„ %d:%s\n",
...
int inter_guild_charname_changed(int guild_id,int account_id, int char_id, char *name)
{
	...
	if( g == NULL )
	{
		ShowError("inter_guild_charrenamed: Cl„ %d n„o foi encontrado\n", guild_id);
		return 0;
	}
	...
	if( i == g->max_member )
	{
		ShowError("inter_guild_charrenamed: N„o foi encontrado o char %d no cl„ %d\n", char_id, guild_id);
		return 0;
	}
...
int mapif_parse_GuildCastleDataSave(int fd, int castle_id, int index, int value)
{
	...
	if (gc == NULL) {
		ShowError("mapif_parse_GuildCastleDataSave: Castelo %d n„o foi encontrado\n", castle_id);
		return 0;
	}
	...
	switch (index) {
		case 1:
			if (log_inter && gc->guild_id != value) {
				...
				inter_log("cl„ %s (id %d) %s castelo de id %d\n",
		...
		default:
			...
			ShowError("mapif_parse_GuildCastleDataSave: Õndice %d n„o foi encontrado\n", index);
			return 0;
	}
...
int mapif_parse_GuildMasterChange(int fd, int guild_id, const char* name, int len)
{
	...
	ShowInfo("int_guild: Mestre do cl„ mudou para %s (Cl„ %d - %s)\n",g->master, guild_id, g->name);
