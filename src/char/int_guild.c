...
#ifdef NOISY
	ShowInfo("Pedido de salvamento do cl„ ("CL_BOLD"%d"CL_RESET" - flag 0x%x).",g->guild_id, flag);
#endif
...
struct guild * inter_guild_fromsql(int guild_id)
{
	...
	{	// Fix reduction of MAX_GUILD [PoW]
		ShowWarning("Cl„ %d:%s possui maior capacidade (%d) do que MAX_GUILD (%d)\n", guild_id, g->name, g->max_member, MAX_GUILD);
...
static struct guild_castle* inter_guildcastle_fromsql(int castle_id)
{
	...
	if (save_log)
		ShowInfo("Castelo "CL_WHITE"%d"CL_RESET" carregado (Cl„ "CL_WHITE"%d"CL_RESET").\n", castle_id, gc->guild_id);
...
int inter_guild_ReadEXP(void)
{
	...
	fp=fopen(line,"r");
	if(fp==NULL){
		ShowError("n„o foi possÌvel ler %s\n", line);
		return 1;
	}
	i=0;
...
int inter_guild_CharOnline(int char_id, int guild_id)
{
   ...
   int i;
	...
	if(!g) {
		ShowError("Cl„ %d da personagem %d n„o encontrado!\n", guild_id, char_id);
		return 0;
	}
...
int guild_calcinfo(struct guild *g)
{
	...
	if(g->max_member > MAX_GUILD)
	{
		ShowError("Cl„ %d:%s tem capacidade para membros de cl„ (%d) acima do limite, o m·ximo suportado È %d\n", g->guild_id, g->name, g->max_member, MAX_GUILD);
		g->max_member = MAX_GUILD;
	}
...
int mapif_guild_noinfo(int fd,int guild_id)
{
	...
	ShowWarning("int_guild: informaÁ„o n„o encontrada %d\n",guild_id);
	if(fd<0)
		mapif_sendall(buf,8);
	else
		mapif_send(fd,buf,8);
...
int mapif_guild_withdraw(int guild_id,int account_id,int char_id,int flag, const char *name, const char *mes)
{
	...
	ShowInfo("int_guild: saÌda de cl„ (%d - %d: %s - %s)\n",guild_id,account_id,name,mes);
	return 0;
}
...
int mapif_guild_broken(int guild_id,int flag)
{
	...
	WBUFB(buf,6)=flag;
	mapif_sendall(buf,7);
	ShowInfo("int_guild: Cl„ desfeito (%d)\n",guild_id);
	return 0;
}
...
int mapif_parse_CreateGuild(int fd,int account_id,char *name,struct guild_member *master)
{
	...
	if(search_guildname(name) != 0){
		ShowInfo("int_guild: cl„ de mesmo nome j· existe [%s]\n",name);
	...
	if (!inter_guild_tosql(g,GS_BASIC|GS_POSITION|GS_SKILL)) {
		...
		ShowError("Falha ao criar o Cl„ %s (LÌder: %s)\n", g->name, g->master);
		mapif_guild_created(fd,account_id,NULL);
		aFree(g);
		return 0;
	}
	ShowInfo("Criando Cl„ %d - %s (LÌder: %s)\n", g->guild_id, g->name, g->master);
...
int mapif_parse_GuildBasicInfoChange(int fd,int guild_id,int type,const char *data,int len)
{
	...
	switch(type)
	{
		case GBI_GUILDLV:
			...
			return 0;
		default:
			ShowError("int_guild: GuildBasicInfoChange: Tipo desconhecido %d\n",type);
			break;
	}
...
int inter_guild_charname_changed(int guild_id,int account_id, int char_id, char *name)
{
	...
	if( g == NULL )
	{
		ShowError("inter_guild_charrenamed: N„o foi possÌvel achar o cl„ %d.\n", guild_id);
		return 0;
	}
	...
	if( i == g->max_member )
	{
		ShowError("inter_guild_charrenamed: N„o foi possÌvel achar a personagem %d no cl„\n", char_id);
		return 0;
	}
...
int mapif_parse_GuildCastleDataSave(int fd, int castle_id, int index, int value)
{
	...
	if (gc == NULL) {
		ShowError("mapif_parse_GuildCastleDataSave: id do castelo=%d n„o encontrado\n", castle_id);
		return 0;
	}
	...
	switch (index) {
		...
		default:
			...
			ShowError("mapif_parse_GuildCastleDataSave: indÌce n„o encontrado=%d\n", index);
			return 0;
	}
...
int mapif_parse_GuildMasterChange(int fd, int guild_id, const char* name, int len)
{
	...
	ShowInfo("int_guild: LÌder do Cl„ alterado para %s (Cl„ %d - %s)\n",g->master, guild_id, g->name);
