...
void set_char_online(int map_id, int char_id, int account_id)
{
	...
	{
		ShowNotice("set_char_online: Personagem ("CL_WHITE"%d"CL_RESET":"CL_WHITE"%d"CL_RESET") marcado no map-server "CL_WHITE"%d"CL_RESET", porÈm o map-server "CL_WHITE"%d"CL_RESET" diz possuir ("CL_WHITE"%d"CL_RESET":"CL_WHITE"%d"CL_RESET") online!\n",
...
{
	if (id < 0)
		ShowNotice("Tornando offline todos os usu·rios.\n");
	else
		ShowNotice("Enviando usu·rios do map-server %d offline.\n",id);
...
int memitemdata_to_sql(const struct item items[], int max, int id, int tableswitch)
{
	...
	default:
		ShowError("Nome de tabela inv·lida!\n");
		return 1;
	}
...
int mmo_char_fromsql(int char_id, struct mmo_charstatus* p, bool load_everything)
{
	...
	{
		ShowError("Requisito de char id %d n„o existente!\n", char_id);
		SqlStmt_Free(stmt);
		return 0;
	}
	p->last_point.map = mapindex_name2id(last_map);
	...
	for( i = 0; i < MAX_SKILL && SQL_SUCCESS == SqlStmt_NextRow(stmt); ++i )
	{
		if( tmp_skill.id < ARRAYLENGTH(p->skill) )
			...
			ShowWarning("mmo_char_fromsql: ignorando habilidade inv·lida (id=%u,lv=%u) da personagem %s (AID=%d,CID=%d)\n", tmp_skill.id, tmp_skill.lv, p->name, p->account_id, p->char_id);
	}
	strcat(t_msg, " skills");
	...
	while( SQL_SUCCESS == SqlStmt_NextRow(stmt) )
	{
		if( hotkey_num >= 0 && hotkey_num < MAX_HOTKEYS )
			...
			ShowWarning("mmo_char_fromsql: ignorando atalho inv·lido (hotkey=%d,type=%u,id=%u,lv=%u) da personagem %s (AID=%d,CID=%d)\n", hotkey_num, tmp_hotkey.type, tmp_hotkey.id, tmp_hotkey.lv, p->name, p->account_id, p->char_id);
	}
...
int mmo_char_sql_init(void)
{
	...
	ShowInfo("Finalizada inicializaÁ„o.\n");
...
int delete_char_sql(int char_id)
{
	...
	{
		ShowError("delete_char_sql: N„o foi possÌvel retornar dados da personagem, remoÁ„o abortada.\n");
	...
	if( ( char_del_level > 0 && base_level >= char_del_level )
	 || ( char_del_level < 0 && base_level <= -char_del_level )
			...
			ShowInfo("RemoÁ„o de personagem abortada: %s, Nv. Base: %i\n", name, base_level);
			return -1;
	}
...
{
	ShowWarning("Conex„o com o login-server perdida.\n\n");
}
	...
	while(RFIFOREST(fd) >= 2)
	{
		...
		case 0x2711:
			...
			if (RFIFOB(fd,2)) {
				...
				ShowError("N„o foi possÌvel conectar-se ao login-server.\n");
				ShowError("As senhas de comunicaÁ„o entre servidores (padr„o: s1/p1) provavelmente est„o incorretas.\n");
				ShowNotice("Por favor, altere a tabela 'login' para criar uma conex„o funcional. (Coloque account_id com o valor '1' e gender 'S')\n");
				ShowNotice("E apÛs, troque os dados da comunicaÁ„o em conf/map_athena.conf e conf/char_athena.conf.\n");
				set_eof(fd);
				return 0;
			} else {
				ShowStatus(" xito na conex„o ao login-server (Conex„o: '"CL_WHITE"%d"CL_RESET"').\n", fd);
				loginif_on_ready();
			}
			RFIFOSKIP(fd,3);
		break;
...
int parse_frommap(int fd)
{
	...
	{// not a map server
		ShowDebug("parse_frommap: Desconectando sess„o inv·lida #%d (n„o È um map-server)\n", fd);
		do_close(fd);
		return 0;
	}
	if( session[fd]->flag.eof )
	...
	while(RFIFOREST(fd) >= 2)
	{
		...
		case 0x2afa: // Receiving map names list from the map-server
			...
			int x;
			if (j == 0) {
				ShowWarning("Map-server "CL_WHITE"%d"CL_RESET" "CL_RED"N√O"CL_RESET" possui mapas.\n", id);
			} else {
				...
				return 0;
		{
			...
			if( Sql_NumRows(sql_handle) > 0 )
			{
				...
				if (count >= 50)
					ShowWarning("Muitas mudanÁas de status para %d:%d, algumas n„o foram carregadas.\n", aid, cid);
				if (count > 0)
				{
				...
				return 0;
		{
			...
			for(i = 0; i < server[id].users; i++) {
				...
				{
					ShowNotice("Set map user: Personagem (%d:%d) marcado no map-server %d, porÈm o map-server %d diz possuir (%d:%d) online!\n",
				...
				return 0;
		{
			...
			{
				ShowError("parse_from_map (save-char): Tamanho n„o coincide! %d != %d\n", size-13, sizeof(struct mmo_charstatus));
				RFIFOSKIP(fd,size);
				break;
			}
				...
				character->char_id == cid))
			{
				...
				ShowError("parse_from_map (save-char): Recebidos dados de personagem n„o existente/offline (%d:%d).\n", aid, cid);
...
//					result = 2; // 2-gm level too low
		...
		case 0x2736: // ip address update
			...
			ShowInfo("Atualizado endereÁo de IP do map-server #%d para %d.%d.%d.%d.\n", id, CONVIP(server[id].ip));
			RFIFOSKIP(fd,6);
		break;
...
int lan_subnetcheck(uint32 ip)
{
	...
	if( i < subnet_count ) {
		ShowInfo("VerificaÁ„o de sub-rede [%u.%u.%u.%u]: Equivale a "CL_CYAN"%u.%u.%u.%u/%u.%u.%u.%u"CL_RESET"\n", CONVIP(ip), CONVIP(subnet[i].char_ip & subnet[i].mask), CONVIP(subnet[i].mask));
		return subnet[i].map_ip;
	} else {
		ShowInfo("VerificaÁ„o de sub-rede [%u.%u.%u.%u]: "CL_CYAN"WAN"CL_RESET"\n", CONVIP(ip));
		return 0;
	}
}
...
{// CH: <0829>.W <char id>.L <birth date:YYMMDD>.6B
	...
	ShowInfo(CL_RED"Pedido de RemoÁ„o de Personagem: "CL_GREEN"%d (%d)"CL_RESET"\n", sd->account_id, char_id);
...
int parse_char(int fd)
{
	...
	while( RFIFOREST(fd) >= 2 )
	{
				...
				return 0;
		{
			...
			ShowInfo("Pedido de conex„o - AID: "CL_WHITE"%d"CL_RESET", login_id1: "CL_WHITE"%d"CL_RESET", login_id2: "CL_WHITE"%d"CL_RESET"\n", account_id, login_id1, login_id2);

			if (sd) {
				//Received again auth packet for already authentified account?? Discard it.
				//TODO: Perhaps log this as a hack attempt?
				//TODO: and perhaps send back a reply?
				ShowNotice("Pacote de autenticaÁ„o recebido novamente, possÌvel tentativa de hack. Ser· descartada.");
				break;
			}
			...
			FIFOSD_CHECK(3);
		{
			...
			if (i < 0 || !cd->last_point.map) {
				...
				if (j == ARRAYLENGTH(server)) {
					ShowInfo("Conex„o Fechada. Nenhum map-server disponÌvel.\n");
					WFIFOHEAD(fd,3);
					WFIFOW(fd,0) = 0x81;
				...
				if ((i = search_mapserver((j=mapindex_name2id(MAP_PRONTERA)),-1,-1)) >= 0) {
					...
					ShowInfo("Conex„o Perdida. Nenhum map-server disponÌvel que possua uma cidade grande, e n„o foi possÌvel achar map-server para '%s'.\n", mapindex_id2name(cd->last_point.map));
					WFIFOHEAD(fd,3);
					WFIFOW(fd,0) = 0x81;
					WFIFOB(fd,2) = 1; // 01 = Server closed
					WFIFOSET(fd,3);
				...
				ShowWarning("N„o foi possÌvel achar o map-server para '%s', enviando para cidade '%s'.\n", mapindex_id2name(cd->last_point.map), mapindex_id2name(j));
				cd->last_point.map = j;
			}
			...
			{
				ShowError("parse_char: Tentativa de escrita em sess„o inv·lida %d! Servidor de Mapas #%d desconectado.\n", map_fd, i);
				server[i].fd = -1;
...
int parse_console(const char* command)
{
	...
	else if( strcmpi("help", command) == 0 )
	{
		...
		ShowInfo("Para saber se o servidor est· ativo:\n");
...
int check_connect_login_server(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	ShowInfo("Tentativa de conex„o com o login-server...\n");
...
int char_lan_config_read(const char *lancfgName)
{
	...
	if((fp = fopen(lancfgName, "r")) == NULL) {
		ShowWarning("Arquivo de configuraÁ„o para Suporte a LAN n„o encontrado: "CL_WHITE"%s"CL_RESET"\n", lancfgName);
		return 1;
	}

	ShowInfo("Lendo arquivo de configuraÁ„o "CL_WHITE"%s"CL_RESET"...\n", lancfgName);
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if(sscanf(line,"%[^:]: %[^:]:%[^:]:%[^\r\n]", w1, w2, w3, w4) != 4) {
			...
			ShowWarning("Erro de sÌntaxe no arquivo de configuraÁ„o "CL_WHITE"%s"CL_RESET" na linha "CL_WHITE"%d"CL_RESET".\n", lancfgName, line_num);
			continue;
		}
		...
		if( strcmpi(w1, "subnet") == 0 )
		{
			...
			{
				ShowError("%s: Erro de ConfiguraÁ„o: O char-server (%s) e map-server (%s) pertencem a diferentes sub-redes!\n", lancfgName, w3, w4);
				continue;
			}
	...
	ShowStatus("Lendo informaÁıes sobre %d sub-redes.\n", subnet_count);
...
void sql_config_read(const char* cfgName)
{
	...
	if ((fp = fopen(cfgName, "r")) == NULL) {
		ShowError("Arquivo n„o encontrado: "CL_WHITE"%s"CL_RESET"\n", cfgName);
		return;
	}
...
int char_config_read(const char* cfgName)
{
	...
	if (fp == NULL) {
		ShowError("Arquivo de configuraÁ„o n„o encontrado: "CL_WHITE"%s"CL_RESET".\n", cfgName);
		return 1;
	}
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if(strcmpi(w1,"timestamp_format") == 0) {
			...
			ShowInfo("ConfiguraÁ„o de silenciosidade do console: %d\n", atoi(w2));
			msg_silent = atoi(w2);
			...
			if (login_ip) {
				...
				ShowStatus("EndereÁo IP do login-server: "CL_WHITE"%s"CL_RESET" -> "CL_WHITE"%s"CL_RESET"\n", w2, ip2str(login_ip, ip_str));
			}
			...
			if (char_ip){
				...
				ShowStatus("EndereÁo IP do char-server: "CL_WHITE"%s"CL_RESET" -> "CL_WHITE"%s"CL_RESET"\n", w2, ip2str(char_ip, ip_str));
			}
			...
			if (bind_ip) {
				...
				ShowStatus("EndereÁo IP do bind do char-server: "CL_WHITE"%s"CL_RESET" -> "CL_WHITE"%s"CL_RESET"\n", w2, ip2str(bind_ip, ip_str));
			}
			...
			if (!start_point.map)
				ShowError("start_point %s especificado n„o foi encontrado no cache do map-index.\n", map);
			start_point.x = x;
			start_point.y = y;
			...
			if (fame_list_size_chemist > MAX_FAME_LIST) {
				ShowWarning("Tamanho m·ximo da lista de fama È %d (fame_list_alchemist)\n", MAX_FAME_LIST);
			...
			if (fame_list_size_smith > MAX_FAME_LIST) {
				ShowWarning("Tamanho m·ximo da lista de fama È %d (fame_list_blacksmith)\n", MAX_FAME_LIST);
			...
			if (fame_list_size_taekwon > MAX_FAME_LIST) {
				ShowWarning("Tamanho m·ximo da lista de fama È %d (fame_list_taekwon)\n", MAX_FAME_LIST);
...
int do_init(int argc, char **argv)
{
	...
	{
		ShowError("N„o foi possÌvel iniciar o emulador.\n");
		ShowError("O servidor est· utilizando senhas de comunicaÁ„o s1/p1. Isto È inv·lido e o servidor n„o ir· iniciar.\n");
		ShowNotice("Por favor, altere a tabela 'login' para criar uma conex„o funcional. (Coloque account_id com o valor '1' e gender 'S')\n");
		ShowNotice("E apÛs, troque os dados da comunicaÁ„o em conf/map_athena.conf e conf/char_athena.conf.\n");
		return 0;
	}

	ShowInfo("Finalizada leitura da configuraÁ„o do char-server.\n");

	inter_init_sql((argc > 2) ? argv[2] : inter_cfgName);
	ShowInfo("Finalizada leitura da configuraÁ„o do inter-server.\n");
	...
	if ((naddr_ != 0) && (!login_ip || !char_ip))
	{
		...
		if (naddr_ > 1)
			ShowStatus("Foram detectadas m˙ltiplas interfaces. Usando "CL_WHITE"%s"CL_RESET" como endereÁo IP\n", ip_str);
		else
			ShowStatus("Padronizando "CL_WHITE"%s"CL_RESET" como endereÁo IP\n", ip_str);
		if (!login_ip) {
	...
	ShowInfo("Final da inicializaÁ„o do char-server.\n");

	set_defaultparse(parse_char);
	char_fd = make_listen_bind(bind_ip, char_port);
	ShowStatus("O char-server est· "CL_GREEN"pronto"CL_RESET" (Funcionando pela porta "CL_GREEN"%d"CL_RESET").\n\n", char_port);
